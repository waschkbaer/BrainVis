...
#include <fstream>
#include <cstdlib> // abs for ubuntu
#include <cstring> // memcpy/memset for ubuntu

#ifdef SMIM_JPG_SUPPORT
#include "SysTools.h"

#ifdef _WIN32
#include "CompressedFormats/JPEGDecoder.h"
#include "CompressedFormats/JPEGEncoder.h"
#else
#include "EAGLConnector.h"
#endif
...
namespace IVDA
{
  ...
  SmallImage::SmallImage(Vec2ui const& size, unsigned int iComponentCount) :
  m_bPremultipliedAlpha(false),
  m_size(size),
  m_iComponentCount(iComponentCount),
  m_pData(0)
  ...
  m_bPremultipliedAlpha(false),
	...
	{
#ifdef SMIM_JPG_SUPPORT
	  if (SysTools::ToLowerCase(SysTools::GetExt(filename)) == "bmp") 
      LoadFromBMPFile(filename);
	  else
      LoadFromJPGFile(filename);
#else
	  LoadFromBMPFile(filename);
#endif
	}
  
  SmallImage::SmallImage(const SmallImage& other) :
  m_bPremultipliedAlpha(other.m_bPremultipliedAlpha),
  m_size(other.m_size),
  m_iComponentCount(other.m_iComponentCount),
  m_pData(0)
  {
    size_t const iDataSize = m_size.area()*m_iComponentCount;
    m_pData = new uint8_t[iDataSize];
    memcpy(m_pData, other.m_pData, iDataSize);
  }

  SmallImage& SmallImage::operator=(const SmallImage& other) {
    if (m_pData)
      delete [] m_pData;
  ...
  void SmallImage::PremultiplyAlpha(bool bTrueFalse) {
    if (bTrueFalse == m_bPremultipliedAlpha)
      return;
    m_bPremultipliedAlpha = bTrueFalse; // set premultiplication flag even if there is no fourth channel
    if (m_iComponentCount != 4)
      return;
    ...
    for (uint32_t x=0; x<m_size.x; ++x)
    {
      for (uint32_t y=0; y<m_size.y; ++y)
      {
        ...
        if (m_bPremultipliedAlpha) {
          ...
          // invert premultiplication if alpha is larger than zero to avoid division by zero
          if (alpha > 0) {
            r = uint8_t(r / alpha);
            g = uint8_t(g / alpha);
            b = uint8_t(b / alpha);
	...
	bool SmallImage::PeekBMPHeader(const std::string& filename, Vec2ui& size, unsigned int& iComponentCount) {
		bool bUpsideDown;
		int iOffsetToData;
		return PeekBMPHeader(filename, size, iComponentCount, bUpsideDown, iOffsetToData);
	}		
  
	bool SmallImage::PeekBMPHeader(const std::string& filename, Vec2ui& size, unsigned int& iComponentCount, bool& bUpsideDown, int& iOffsetToData) {    
...
#ifdef SMIM_JPG_SUPPORT
	bool SmallImage::LoadFromJPGFile(const std::string& filename) {
	  // seek to the actual data
	  std::ifstream inStream(filename.c_str(), std::ofstream::binary);
	  if (inStream.fail()) return false;
    
	  std::streampos begin = inStream.tellg();
	  inStream.seekg (0, std::ios::end);
	  std::streampos end = inStream.tellg();
	  inStream.seekg (0, std::ios::beg);
    
	  std::vector<uint8_t> jpegData;
	  jpegData.resize(size_t(end-begin));
	  inStream.read((char*)&jpegData[0], end-begin);
	  inStream.close();
    
	  return LoadFromJPGMem(jpegData);
	}
  
	bool SmallImage::LoadFromJPGMem(std::vector<uint8_t> const& vData) {
#ifdef _WIN32    
	  JPEGDecoder jpegDecoder;
	  PixelsResolution pixelsResolution(m_size.x, m_size.y);
	  unsigned long pixelsBytes = 0;
	  PixelFormat pixelFormat = PF_UNKNOWN;
	  const std::vector<uint8_t> & decodedPixels = jpegDecoder.getPixels(vData, pixelFormat, pixelsResolution, pixelsBytes);
    
	  if (decodedPixels.size() == 0) return false;
    
	  m_size.x = pixelsResolution.x;
	  m_size.y = pixelsResolution.y;
	  m_iComponentCount = 3;
	  delete [] m_pData;
	  m_pData = new uint8_t[m_size.area()*m_iComponentCount];
	  memcpy(m_pData, &decodedPixels[0], m_size.area()*m_iComponentCount);	
	  return true;
#else
    size_t sizeU = 0, sizeV = 0, bufferSize = 0;

    delete [] m_pData;
  
    EAGLConnector::ImageDataFromBuffer((void*)&vData[0], vData.size(), sizeU, sizeV, (void**)&m_pData, bufferSize, false); 
	...
	bool SmallImage::SaveToJPGMem(std::vector<uint8_t>& vData) const {
#ifdef _WIN32    
	  JPEGEncoder jpegEncoder;
	  std::vector<uint8_t> const rawPixels(m_pData, m_pData + m_size.area()*m_iComponentCount);
	  unsigned long bytesWritten;
	  PixelsResolution pixelsResolution(m_size.x, m_size.y);
	  std::vector<uint8_t> const& jpegData = jpegEncoder.getJPEG(rawPixels, (m_iComponentCount == 4 ? PF_RGBA : PF_RGB), pixelsResolution, bytesWritten);
    
	  if (jpegData.size() == 0) return false;
	  if (bytesWritten == 0) return false;
	  if (jpegData.size() < bytesWritten) return false;
    
	  vData.resize(bytesWritten);
	  memcpy(&vData[0], &jpegData[0], bytesWritten);
	  return true;
#else
    // currently unsupported
    // but should work with EAGLConnector::JPEGFromBuffer
    return false;
#endif
	}
  
	bool SmallImage::SaveToJPGFile(const std::string& filename) const {
	  std::ofstream outStream(filename.c_str(), std::ofstream::binary);
	  if (!outStream.is_open()) return false;
    
	  std::vector<uint8_t> jpegData;
	  if (!SaveToJPGMem(jpegData)) return false;
    
	  outStream.write((char*)&jpegData[0], jpegData.size());
	  outStream.close();
	  return true;
	}
#endif
  
	bool SmallImage::LoadFromBMPFile(const std::string& filename) {    
	  bool bUpsideDown;
	  int iOffsetToData;
	  Vec2ui size;
	  unsigned int iComponentCount;
	  if (!PeekBMPHeader(filename, size, iComponentCount, bUpsideDown, iOffsetToData)) {
      return false;
	  }
	  ...
	  // reuse acquired data if possible
	  if (size != m_size || iComponentCount != m_iComponentCount) {
      m_size = size;
      m_iComponentCount = iComponentCount;
      delete m_pData;
      m_pData = new uint8_t[m_iComponentCount*m_size.area()];
    ...
    // if premultiply alpha is set we need to apply it to the newly loaded image
    if (m_bPremultipliedAlpha) {
      m_bPremultipliedAlpha = false;
	...
	void SmallImage::Resample(unsigned int newWidth, unsigned int newHeight, bool bKeepAspect) {
	  ...
	  delete [] m_pData;
	  m_pData = pData;
	  m_size = Vec2ui(newWidth, newHeight);
	}
	...
	void SmallImage::GetPixel(unsigned int x, unsigned int y, uint8_t& r, uint8_t& g, uint8_t& b, uint8_t& a) const {    
	  ...
	  g = m_pData[index+1];
	  b = m_pData[index+2];
	  a = (m_iComponentCount == 4) ? m_pData[index+3] : 255;
	}
